...
export class UtilsReportsService {
  async generarReporteSimple(nombreHoja: string, columnas: string[], datos: any[], res: Response, nombreArchivo: string) {
    const workbook = await XlsxPopulate.fromBlankAsync();
    XlsxHelper.crearHoja(workbook, nombreHoja, columnas, datos);
    return XlsxHelper.exportar(workbook, res, nombreArchivo);
  }

  async generarReporteMultiHojas(reportes: { nombreHoja: string; columnas: string[]; datos: any[] }[], res: Response, nombreArchivo: string) {
    const workbook = await XlsxPopulate.fromBlankAsync();
    reportes.forEach(r => {
